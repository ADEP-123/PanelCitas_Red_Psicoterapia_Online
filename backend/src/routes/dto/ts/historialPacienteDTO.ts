...
export class historialPacienteDTO {
    ...
    @Transform(({ value, key }) => { if (/^[0-9]+$/.test(value)) return value; else throw { status: 400, message: `Error en la escritura del documento de identificacion del paciente` } }, { toClassOnly: true })
    paciente: string;
